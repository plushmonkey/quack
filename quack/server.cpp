...
static void SendFrame(QuackSocket socket, OpCode opcode, std::string_view data) {
      ...
      case SessionState::Upgraded: {
        ...
        if (current_frame_header.flags & FrameHeaderFlag_Fin) {
          switch (current_frame_header.opcode) {
            case OpCode::Text:
            case OpCode::Binary: {
              server->OnMessage(socket, payload);
            } break;
            case OpCode::Close: {
              ...
              SendFrame(socket, OpCode::Close, std::string_view((char*)&close_code, sizeof(close_code)));

              return ProcessResult::Disconnect;
            } break;
            case OpCode::Ping: {
              SendFrame(socket, OpCode::Pong, payload);
            } break;
            case OpCode::Pong: {
            } break;
            default: {
...
bool OnRecv(ConnectionUserData user, char* data, size_t size) {
...
ConnectionUserData OnAccept(ServerUserData user, QuackSocket socket) {
  Session* session = new Session;

  session->server = (Server*)user;
  session->socket = socket;

  session->server->OnOpen(socket);

  return session;
}

void OnClose(ConnectionUserData user) {
  Session* session = (Session*)user;

  session->server->OnClose(session->socket);

  delete session;
}

Server::Server() {
  processor.accept_callback = OnAccept;
  processor.recv_callback = OnRecv;
  processor.close_callback = quack::OnClose;
  processor.server_user_data = this;
}

void Server::Send(QuackSocket socket, std::string_view data) {
  SendFrame(socket, OpCode::Text, data);
}
